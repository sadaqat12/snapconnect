...
interface FriendWithSnaps extends Friend {
...
export default function FriendsScreen() {
  ...
  const [friendsWithSnaps, setFriendsWithSnaps] = useState<FriendWithSnaps[]>([]);
  ...
  const loadSnaps = async () => {
    try {
      ...
      // Group snaps by sender and get last contact time
      const friendsMap = new Map<string, FriendWithSnaps>();
      ...
      const sortedFriends = Array.from(friendsMap.values()).sort((a, b) => {
        // First sort by unread snaps
        if (a.unreadSnaps.length !== b.unreadSnaps.length) {
          return b.unreadSnaps.length - a.unreadSnaps.length;
        }
      ...
      console.log('Processed friends with snaps:', sortedFriends.map(f => ({
        id: f.id,
        username: f.username,
        unreadCount: f.unreadSnaps.length,
        snaps: f.unreadSnaps.map(s => ({
          id: s.id,
      ...
      setFriendsWithSnaps(sortedFriends);
    } catch (error) {
